    ...
    def can_move(self, coordinates: Coordinates, target: Coordinates) -> bool:
        return (
                ((abs(coordinates.x - target.x) == 2 and abs(coordinates.y - target.y) == 1)
                 or (abs(coordinates.x - target.x) == 1 and abs(coordinates.y - target.y) == 2))
                and coordinates != target
        )
...
class Pawn(Piece):
    def can_move(self, coordinates: Coordinates, target: Coordinates) -> bool:
        if coordinates != target:
            if self.color == Colors.WHITE and coordinates.x == target.x and target.y - coordinates.y == 1:
                return True
            if self.color == Colors.BLACK and coordinates.x == target.x and target.y - coordinates.y == -1:
                return True
        return False
